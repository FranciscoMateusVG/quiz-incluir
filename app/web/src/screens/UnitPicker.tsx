...
export function UnitPicker() {
  ...
  if (units.length === 0) {
    return <EmptyState icon={Layers} title={t.noUnits} hint={t.noUnitsHint} />;
  }
      ...
      >
        {units.map(({ unit, quizzes: unitQuizzes }) => (
          <button
            key={unit}
            type="button"
            onClick={() => void navigate(`/units/${encodeURIComponent(unit)}`)}
            className={cn(
              // min-w-0: same grid-blowout fix as QuizCard — this button is a
              // direct grid item, and its nowrap (truncate) label would
              // otherwise set the item's min-width and stretch the grid
              // track past the viewport on mobile.
